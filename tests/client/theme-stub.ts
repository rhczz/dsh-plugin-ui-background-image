/**
 * Stand-in for the client theme service, reproducing the two facts the
 * background presenter depends on: the service owns the document write, and it
 * keeps one token layer per source that a later call replaces and a disposer
 * removes.
 *
 * The write itself mirrors ui-layout's theme presenter — inline CSS variables
 * on `body`, retracting exactly the names it wrote last time — so a spec asserts
 * against the document a browser would render, not against a call log.
 * @module dsh-plugin-ui-background-image/tests/client/theme-stub
 */

import type { ThemeTokenWriter } from '../../src/client/background-presenter.ts'
